...
test("EF-207__Unarchive Forms Functionality", async ({ page }) => {
  ...
  await expect(page.url()).toBe(
    "https://app.easyfleet.ai/inspections/forms/?status=archived"
  );

  const unarchivedRow = await page
    .getByRole("row")
    .locator(Selectors.dataCell)
    .nth(0)
    .innerText();
  ...
  await expect(
    page.locator('[id="\\31 "]').getByText("Unarchived")
  ).toBeVisible();
  ...
  await expect(page.url()).toBe(
    "https://app.easyfleet.ai/inspections/forms/?status=active"
  );
