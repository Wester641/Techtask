...
test("EF-93__Verify Search Field and Vehicle Filter", async ({ page }) => {
  ...
  const meterValues = apiResponseData.results.map(item => item.meter_value).filter(Boolean);

  const randomMeterValue = meterValues[Math.floor(Math.random() * meterValues.length)];

  const vehicles = apiResponseData.results.map(item => item.vehicle?.name).filter(Boolean);

  const filteredVehicles = vehicles.filter(vehicle => !vehicle.includes("Truck")); // Except "Truck..." vehicles due to Bug with filter
  ...
  await expect(page.getByRole('cell', { name: randomMeterValue, exact: true }).first()).toHaveText(String(randomMeterValue));
  ...
  await page.locator(Selectors.filterButton).nth(1).click();

  await expect(page.getByRole('cell', { name: randomVehicle, exact: true }).first()).toHaveText(randomVehicle);
});